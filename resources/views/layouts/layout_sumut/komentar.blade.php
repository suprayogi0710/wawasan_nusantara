        ...
        <div class="card">
                        ...
                        @foreach ($province_comments->comments as $firstLevelComment)
                            <div class="d-flex flex-start">
                                <img class="rounded-circle shadow-1-strong me-3"
                                    src="https://ui-avatars.com/api/?name={{ urlencode($firstLevelComment->user_name) }}&background=random" alt="avatar" width="45"
                                    height="45" />
                                <div class="flex-grow-1 flex-shrink-1">
                                    <div>
                                        <div style="text-align: left">
                                            <h6 class="fw-bold text-primary mb-1">{{ $firstLevelComment->user_name }}
                                            </h6>
                                            ...
                                            <div class="small d-flex justify-content-start">
                                                ...
                                                <a class="d-flex align-items-center me-3 comment-button" role="button" data-provinceid="{{ $firstLevelComment->province_id }}"
                                    ...
                                    @foreach ($firstLevelComment->replies as $secondLevelComment)
                                        <div class="d-flex flex-start mt-4 mb-3">
                                            <img class="rounded-circle shadow-1-strong me-3"
                                                src="https://ui-avatars.com/api/?name={{ urlencode($secondLevelComment->user_name) }}&background=random" alt="avatar"
                                                width="45" height="45" />
                                            <div class="flex-grow-1 flex-shrink-1">
                                                <div>
                                                    <div style="text-align: left">
                                                        <h6 class="fw-bold text-primary mb-1">
                                                            {{ $secondLevelComment->user_name }}</h6>
                                                        <h6 class="text-muted mb-0">
                                                            {{ $carbon::parse($secondLevelComment->created_at)->format('l, d M Y') }}
                                                        </h6>

                                                        <p id="{{ $secondLevelComment->id }}" class="small text-black-50 my-3" style="text-align: left;">
                                                            {{ $secondLevelComment->comment }}
                                                        </p>

                                                        <div class="small d-flex justify-content-start">
                                                            <a id="saveLikeDislike" class="d-flex align-items-center me-3" data-type="like"
                                                                data-province="{{ $secondLevelComment->province_id }}" data-comment="{{ $secondLevelComment->id }}"
                                                                role="button">
            ...
            <div class="card-footer py-3 border-0" style="background-color: #f8f9fa;">
                <form action="{{ url('comment/store') }}" method="post">
                    @csrf
                    <input type="number" name="province_id" hidden value="2">
                    ...
                    <div class="d-flex flex-start w-100">
                        <div class="form-outline w-100" style="text-align: left">
                            <textarea name="comment" class="form-control" id="textAreaExample" rows="4" style="background: #fff;" placeholder="Masukkan komentar disini..."></textarea>
                        </div>
                    </div>
                ...
                <div class="reply" style="display: none">
                    <form class="mb-2" method="post" id="formReply" role="form">
                        ...
                        <div class="d-flex flex-start w-100">
                            <div class="form-outline w-100" style="text-align: left">
                                <textarea name="reply_txt" class="form-control" id="textAreaExample" rows="4" style="background: #fff;" placeholder="Masukkan komentar disini..."></textarea>
                                @error('reply_txt')
                                    <h6 class="text-small text-danger">{{ $message }}</h6>
                                @enderror
                            </div>
    ...
    <script>
        $(document).ready(function() {
            ...
            $('.comment-button').click(function(e) {
                ...
                let [commentId, provinceId] = [$(this).attr('data-commentid'), $(this).attr(
                    'data-provinceid')];
