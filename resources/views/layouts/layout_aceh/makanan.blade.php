 <!--card1-->
 <div class="section-title">
   <h1 class="text pt-3">Makanan Khas Nanggroe Aceh Darusallam</h1>
   <p class="text fs-5">Makanan Khas daerah Nanggroe Aceh Darusallam adalah Makanan Khas yang sudah turun temurun dari nenek moyang pendahulu sehingga memberikan cita rasa yang khas</p>
 </div>
 <div class="card-group">
   <div class="card m-1">
     <img src="/assets/img/indonesia/aceh/makanan/Aceh-Asam_Keueng.jpg" class="card-img-top" alt="..." width="20">
     <div class="card-body">
       <div class="d-grid gap-2">
         <h3 class="text text-center">Masakan Asam Keueng</h3>
         <a href="https://youtu.be/V66k4SffopI?si=_3_weq8sihUhq6OE" button class="btn btn-danger" type="button">Play</a>
       </div>
     </div>
   </div>
   <div class="card m-1">
     <img src="/assets/img/indonesia/aceh/makanan/aceh-ayam_tangkap.jpg" class="card-img-top" alt="..." width="20">
     <div class="card-body">
       <div class="d-grid gap-2">
         <h3 class="text text-center">Masakan Ayam Tangkap</h3>
         <a href="https://youtu.be/dUYuvKbADW8?si=jEMH7kZqCrfbxSWj" button class="btn btn-danger" type="button">Play</a>
       </div>
     </div>
   </div>
   <div class="card m-1">
     <img src="/assets/img/indonesia/aceh/makanan/aceh-Keumamah.jpg" class="card-img-top" alt="..." width="20">
     <div class="card-body">
       <div class="d-grid gap-2">
         <h3 class="text text-center">Masakan Keumamah</h3>
         <a href="https://youtu.be/wVqY8ULUKuw?si=ntgjHbUKx3lWPEEy" button class="btn btn-danger" type="button">Play</a>
       </div>
     </div>
   </div>
   <div class="card m-1">
     <img src="/assets/img/indonesia/aceh/makanan/aceh-Eungkot_Paya.jpg" class="card-img-top" class="card-img-top" alt="..." width="20">
     <div class="card-body">
       <div class="d-grid gap-2">
         <h3 class="text text-center">Masakan Eungkot</h3>
         <a href="https://youtu.be/CTc0oFYr74g?si=IIGWGaXsO1eHvJaq" button class="btn btn-danger" type="button">Play</a>
       </div>
     </div>
   </div>
   <div class="card m-1">
     <img src="/assets/img/indonesia/aceh/makanan/aceh-Kuah_Beulangong.jpg" class="card-img-top" alt="..." width="20">
     <div class="card-body">
       <div class="d-grid gap-2">
         <h3 class="text text-center">Masakan Kuah Beulangong</h3>
         <a href="https://youtu.be/RVq1P52N9mg?si=bZ-u6U9EzpAakAS2" button class="btn btn-danger" type="button">Play</a>
       </div>
     </div>
   </div>
 </div>

 <!--card2-->
 <div class="card-group">
   <div class="card m-1">
     <img src="/assets/img/indonesia/aceh/makanan/aceh-Kuah_Pliek_U.jpg" class="card-img-top" alt="..." width="20">
     <div class="card-body">
       <div class="d-grid gap-2">
         <h3 class="text text-center">Masakan Kuah Pliek U</h3>
         <a href="https://youtu.be/VN2kpdTkFbg?si=bqulEZt4iu0vRkSk" button class="btn btn-danger" type="button">Play</a>
       </div>
     </div>
   </div>
   <div class="card m-1">
     <img src="/assets/img/indonesia/aceh/makanan/aceh-Kuah_Sie_Itek.jpg" class="card-img-top" alt="..." width="20">
     <div class="card-body">
       <div class="d-grid gap-2">
         <h3 class="text text-center">Masakan Kuah Sie Itek</h3>
         <a href="https://youtu.be/yTU-Q_nN6IU?si=vgbWbCnvVImxEcXV" button class="btn btn-danger" type="button">Play</a>
       </div>
     </div>
   </div>
   <div class="card m-1">
     <img src="/assets/img/indonesia/aceh/makanan/aceh-Kue Adee.jpg" class="card-img-top" alt="..." width="20">
     <div class="card-body">
       <div class="d-grid gap-2">
         <h3 class="text text-center">Masakan Kue Adee</h3>
         <a href="https://youtu.be/-EQjRvLUnAU?si=K1N-BXJLvnliC56c" button class="btn btn-danger" type="button">Play</a>
       </div>
     </div>
   </div>
   <div class="card m-1">
     <img src="/assets/img/indonesia/aceh/makanan/aceh-Sambal_Ganja.jpg" class="card-img-top" alt="..." width="20">
     <div class="card-body">
       <div class="d-grid gap-2">
         <h3 class="text text-center">Masakan Sambal Ganja</h3>
         <a href="https://youtu.be/br5f-TwoRvs?si=krHFMK-p5MnjaOB0" button class="btn btn-danger" type="button">Play</a>
       </div>
     </div>
   </div>
   <div class="card m-1">
     <img src="/assets/img/indonesia/aceh/makanan/aceh-Sie_Reuboh.jpg" class="card-img-top" alt="..." width="20">
     <div class="card-body">
       <div class="d-grid gap-2">
         <h3 class="text text-center">Masakan Sie Reuboh</h3>
         <a href="https://youtu.be/voAmVv8D_gg?si=jm1dQmILICuEmlNZ" button class="btn btn-danger" type="button">Play</a>
       </div>
     </div>
   </div>
 </div>